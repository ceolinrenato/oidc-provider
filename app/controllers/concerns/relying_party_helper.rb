module RelyingPartyHelper
  ...
  def set_relying_party_by_client_id
    raise CustomExceptions::InvalidRequest, 5 unless params[:client_id]
    @relying_party = RelyingParty.find_by client_id: params[:client_id]
    raise CustomExceptions::InvalidClient unless @relying_party
  end

  def authenticate_relying_party
    raise CustomExceptions::InvalidRequest, 24 unless params[:client_id] && params[:client_secret]
