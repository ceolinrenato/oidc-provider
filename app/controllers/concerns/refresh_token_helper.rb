module RefreshTokenHelper
  ...
  def set_refresh_token
    raise CustomExceptions::InvalidRequest, 30 unless params[:refresh_token]
    @refresh_token = RefreshToken.find_by(token: params[:refresh_token]).try(:lock!)
    raise CustomExceptions::InvalidGrant, 31 unless @refresh_token
    @device = @refresh_token.access_token.session.device
    raise CustomExceptions::CompromisedDevice if @refresh_token.used
    raise CustomExceptions::InvalidGrant, 31 unless @refresh_token.access_token.relying_party == @relying_party
  end
